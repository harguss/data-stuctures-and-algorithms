...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    ...
    let indexOfPound = ingredient.indexOf(' pound');
    let indexOfGallons = ingredient.indexOf(' gallons');
    if (indexOfPound !== -1) {
      result.push(ingredient.slice(indexOfSpace + 1, indexOfPound));
    } else if (indexOfGallons !== -1) {
      result.push(ingredient.slice(indexOfSpace + 1, indexOfGallons));
    } else {
      result.push(ingredient.slice(indexOfSpace + 1));
    }
  }
  return result;
};
