...
const validatePin = (pin) => {
  const pattern = /^\d{4}$/; // regular expression pattern for four numerical digits
  return pattern.test(pin); // return whether the pin matches the pattern
};
...
const findTagNames = elements => {
  const pattern = /<\/(\w+)>/g; // regular expression pattern to match closing tags
  const closingTags = elements.map(element => element.match(pattern)); // extract closing tag names
  return closingTags.reduce((acc, tags) => acc.concat(tags), []); // flatten array of arrays
};
